...
import hashlib
import time
from typing import Callable, List, Set, Dict, Any, Optional, Tuple
...
class Mutator:
    ...
    def __init__(self, seed: Optional[int] = None):
        ...
        self.interesting_16bit = [-32768, -129, 128, 255, 256, 512, 1000, 1024, 4096, 32767]
        self.interesting_32bit = [-2147483648, -100663046, -32769, 32768, 65535, 65536, 100663045, 2147483647]
        
        self.dictionary = [
            b"GET", b"POST", b"admin", b"root", b"password",
            b"<script>", b"' OR '1'='1", b"../../../",
            b"SELECT", b"UNION", b"DROP", b"INSERT",
            b"<?xml", b"<!DOCTYPE", b"javascript:",
            b"%00", b"%0d%0a", b"\x00", b"\r\n"
        ]
    ...
    def interesting_value_mutation(self, data: bytes) -> bytes:
        ...
        data_array[idx:idx+4] = value.to_bytes(4, byteorder='little', signed=True)
    ...
    def mutate(self, data: bytes, strategy: MutationStrategy) -> bytes:
        ...
        strategies = {
            MutationStrategy.BIT_FLIP: lambda: self.bit_flip(data, self.random.randint(1, 8)),
            MutationStrategy.BYTE_FLIP: lambda: self.byte_flip(data, self.random.randint(1, 4)),
            MutationStrategy.ARITHMETIC: lambda: self.arithmetic_mutation(data),
            MutationStrategy.INTERESTING_VALUES: lambda: self.interesting_value_mutation(data),
...
class CoverageGuidedFuzzer:
    ...
    def execute_target(self, data: bytes) -> Tuple[bool, Optional[Exception], str, float]:
...
if __name__ == "__main__":
    ...
    fuzzer2 = CoverageGuidedFuzzer(
        target_function=vulnerable_calculator,
        timeout=0.1,
        max_iterations=1000
    )
