...
class APISecurityTester:
    ...
    def test_bola_idor(self, endpoint: APIEndpoint, user_id_param: str) -> APITestResult:
        ...
        if user_id_param in endpoint.path:
            ...
            for test_id in test_ids:
                modified_path = endpoint.path.replace(f"{{{user_id_param}}}", str(test_id))
    ...
    def test_authentication_bypass(self, endpoint: APIEndpoint) -> APITestResult:
        ...
        for invalid_token in invalid_tokens:
            response = self._make_request(
                endpoint,
                override_headers={'Authorization': invalid_token}
            )
            if response['status_code'] != 401:
        ...
        if self.auth_token and self.auth_token.startswith('eyJ'):
            try:
                ...
                response = self._make_request(
                    endpoint,
                    override_headers={'Authorization': f'Bearer {none_token}'}
                )
                if response['status_code'] != 401:
    ...
    def test_excessive_data_exposure(self, endpoint: APIEndpoint) -> APITestResult:
        ...
        sensitive_fields = [
            ...
            'token', 'private_key', 'salt', 'hash', 'internal_id'
        ]
    ...
    def test_rate_limiting(self, endpoint: APIEndpoint, requests_count: int = 100) -> APITestResult:
        """
    ...
    def test_function_level_authorization(self, endpoint: APIEndpoint, admin_only: bool = True) -> APITestResult:
        """
        ...
        if admin_only:
            ...
            admin_paths = [
                '/admin', '/api/admin', '/users/all', '/api/users/delete',
                '/settings', '/config', '/debug', '/internal'
            ]
        ...
        for injection_type, payloads in injection_payloads.items():
            for payload in payloads:
                ...
                try:
                    ...
                    vulnerabilities.append(f"{injection_type}: {payload} (caused exception)")
                ...
                if endpoint.body:
                    ...
                    try:
                        ...
                        response_str = json.dumps(response).lower()
                        if any(ind in response_str for ind in indicators):
                            vulnerabilities.append(f"{injection_type} in body: {payload}")
                    except Exception:
                        pass
    ...
    def test_comprehensive(self, endpoints: List[APIEndpoint]) -> List[APITestResult]:
        ...
        for endpoint in endpoints:
            ...
            for test in tests:
                ...
                print(f"  {status} {severity} {test.vulnerability_type.value}: {test.details}")
        ...
        for severity in ['critical', 'high', 'medium', 'low']:
            ...
            if findings:
                ...
                for finding in findings:
                    ...
                    report.append(f"  Endpoint: {finding.endpoint.method} {finding.endpoint.path}")
